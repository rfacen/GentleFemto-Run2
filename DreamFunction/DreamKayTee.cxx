...
void DreamKayTee::ObtainTheCorrelationFunctionAncestors(const char* outFolder,
                                               const char* prefix,
                                               const char* pair, const char* isAlabama) {
  if (!fSEMEReweighting) {
    std::cout << "No Reweighting set. Call SetSEMEReweightingRatio first! \n";
  } else {
    const char* variable = (fIskT) ? "kT" : "mT";
    ...
    if (fSEkT[0]) {
      ...
      this->AveragekT(pair);
      fSum = new DreamCF*[fNKayTeeBins];
      TString outname = TString::Format("%s/AveragemT_%s.root",
                                        outFolder,isAlabama);
      TFile* allCFsOut = TFile::Open(outname.Data(), "UPDATE");
      allCFsOut->cd();
      if (fAveragekT) {
        TString name = TString::Format("Average%s_%s_%s", variable,isAlabama,pair);
        TGraphErrors* copy = new TGraphErrors(*fAveragekT);
        copy->GetXaxis()->SetTitle("m_{T} Bin");
        copy->GetYaxis()->SetTitle("<m_{T}> (GeV/c^{2})");
        copy->SetName(name.Data());
      ...
      for (int ikT = 0; ikT < fNKayTeeBins - 1; ++ikT) {
        ...
        TString outfileName = TString::Format("%s/CFOutput_%s_%s_%s_%s_%u.root",
                                              outFolder, variable, pair, prefix, isAlabama,
                                              ikT);
        for (auto &it : CFs) {
          TString HistName = it->GetName();
          for (int iBin = 1; iBin < it->GetNbinsX(); ++iBin) {
...
void DreamKayTee::ObtainTheCorrelationFunctionAncestorsSingle(const char* outFolder,
                                               const char* prefix,
                                               const char* pair, const char* isAlabama) {
  if (!fSEMEReweighting) {
    std::cout << "No Reweighting set. Call SetSEMEReweightingRatio first! \n";
