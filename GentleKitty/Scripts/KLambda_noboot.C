...
auto funptr = fit_functionLedn;
/// =====================================================================================
//total fit
double fit_functionLedn_pol3(double* x, double* par) {
    double t = *x;
  ...
  double yval= par[3]*FITTER_DECOMLedn->EvalCk(t)*(par[4]*fTemplate_MC_Common->Eval(t) + (1. - par[4])*fTemplate_MC_NonCommon->Eval(t) + (par[5]+par[6]*t*t+par[7]*t*t*t));
  return yval;
}
...
int main(int argc, char* argv[]) {
  ...
  for(int j = 0; j < boot; j++){
    ...
    fTemplate_MC_NonCommon->SetMarkerStyle(kOpenCircle);
    fTemplate_MC_NonCommon->SetMarkerSize(0.3);
    ...
    auto legend = new TLegend(0.55, 0.7, 0.88, 0.88, NULL, "brNDC");
    legend->AddEntry(fTemplate_MC_Common, "Common Ancestors", "l");
    legend->AddEntry(fTemplate_MC_NonCommon, "Non common Ancestors", "l");
    legend->SetTextSize(gStyle->GetTextSize()*0.65);
    legend->SetBorderSize(0);
    legend->Draw();
    gStyle->SetOptStat(0);
    fTemplate_MC_NonCommon->GetXaxis()->SetLabelSize(0.048);
    fTemplate_MC_NonCommon->GetYaxis()->SetLabelSize(0.048);
    fTemplate_MC_NonCommon->GetXaxis()->SetTitleSize(0.048);
    fTemplate_MC_NonCommon->GetYaxis()->SetTitleSize(0.048);
    c_common_fit->Print(TString::Format("%s/Common_fit_%s.pdf", path_results, pair[i]));


    //prefit of the backg round (with pol1 and pol2): i have 4 parameters free, and I take the template from common and uncommom
    ...
    double fit_prefit_min = 400;
    h_HM->Fit(fPreFit_pol1, "S, N, R, M", "", fit_prefit_min, 2000); 
    ...
    h_HM->Fit(fPreFit_pol2, "S, N, R, M", "", fit_prefit_min, 2000);
    ...
    h_HM->Draw("");
    fPreFit_pol2->Draw("same");
    fPreFit_pol1->Draw("same");
    ...
    legend2->AddEntry(fPreFit_pol1, "Full prefit with Pol1");
    legend2->AddEntry(fPreFit_pol2, "Full prefit with Pol2");
    legend2->SetTextSize(gStyle->GetTextSize()*0.6); 
    legend2->Draw();
    fPreFit_pol2->SetLineColor(kGreen + 2);
    fPreFit_pol1->SetLineColor(kMagenta +2);
    fPreFit_pol1->SetLineWidth(2);
  

    TF1* fPreFit_pol3 = new TF1("fPreFit_pol3", PreFit_pol3, 0, 2000, 5);
    fPreFit_pol3->SetParameter(0, fPreFit_pol2->GetParameter(0)); //N_d
    fPreFit_pol3->SetParameter(1, fPreFit_pol2->GetParameter(1)); //w_c
    fPreFit_pol3->SetParLimits(1, 5e-2, 1);
    fPreFit_pol3->SetParameter(2, fPreFit_pol2->GetParameter(2)); //a
    fPreFit_pol3->SetParameter(3, fPreFit_pol2->GetParameter(4)); //c
    fPreFit_pol3->SetParNames("N_d", "w_c", "a", "c", "d");
    fPreFit_pol3->SetLineColor(kOrange + 2); 
    std::cout << "\n---------------------------------\n Prefit with pol3 \n--------------------------------\n" << endl;

    h_HM->Fit(fPreFit_pol3, "S, N, R, M", "", fit_prefit_min, 2000);
    double chi_prefit_pol3 = fPreFit_pol3->GetChisquare()/fPreFit_pol3->GetNDF();
    std::cout<<"Chi2 reduced prefit pol3: "<< chi_prefit_pol3 <<endl;


    outfile->cd();
    fPreFit_pol2->Write("prefit pol2");
    fPreFit_pol1->Write("prefit pol1");
    fPreFit_pol3->Write("prefit pol3");
    ...
    double effrange = 0.0;
    double lambda = 9.83264e-01;
    double w1 = 7.57313e-01;
    ...
    CATSparameters source_func(CATSparameters::tSource, 4, true);
    catsXi.SetAnaSource(NormDoubleGaussSource, source_func);
    catsXi.SetAutoNormSource(false);
    catsXi.SetNormalizedSource(true);
    catsXi.SetAnaSource(0, r1);
    catsXi.SetAnaSource(1, r2);
    catsXi.SetAnaSource(2, w1);
    catsXi.SetAnaSource(3, lambda);
    ...
    catsXi.SetAnaSource(0, r1*w1 + r2*(1-w1));
    catsXi.KillTheCat();  
    ...
    //LKModel
    DLM_Ck *LKModel = new DLM_Ck(4, 3, NumMomBins, kMin, kMax, ComplexLednicky_Singlet_doublegaussian_lambda); //number of Source Par, number of scattering parameters
    LKModel->SetSourcePar(0, r1);
    LKModel->SetSourcePar(1, r2);
    LKModel->SetSourcePar(2, w1);
    LKModel->SetSourcePar(3, lambda);
    LKModel->SetPotPar(0, scattlenRE);
    LKModel->SetPotPar(1, scattlenIM);
    LKModel->SetPotPar(2, effrange);
    LKModel->Update(); 

    //momentum smearing rew
    ...
    if(pol2BL == true){  
      ...
      TF1* fitter = new TF1("fitter", fit_functionLedn, 0, fit_max, NumberOfFitPars);            
      ...
      const double onesigma = TMath::Erf(1 / TMath::Sqrt(2));
      TH1D *hint = new TH1D("hint", "Fitted gaussian with .95 conf.band", h_HM->GetNbinsX(), h_HM->GetXaxis()->GetBinLowEdge(1), h_HM->GetXaxis()->GetBinUpEdge(h_HM->GetNbinsX()));
      (TVirtualFitter::GetFitter())->GetConfidenceIntervals(hint, onesigma);
      hint->Write("conf int");
      //Now the "hint" histogram has the fitted function values as the
      //bin contents and the confidence intervals as bin errorshint->Write("conf interval");

      TF1* fitter_eff_range = new TF1("fitter_eff_range", fit_functionLedn, 0, 500, NumberOfFitPars);            
      fitter_eff_range->SetParameter(0, scattlenRE);
      fitter_eff_range->SetParameter(1, scattlenIM); 
      fitter_eff_range->SetParLimits(1, 0, 100);           
      fitter_eff_range->FixParameter(2, effrange);
      fitter_eff_range->SetParameter(3, fPreFit_pol2->GetParameter(0)); //constant of normalization 
      fitter_eff_range->SetParameter(4, fPreFit_pol2->GetParameter(1)); //weight of common 
      fitter_eff_range->SetParLimits(4, 0, 1);                  
      fitter_eff_range->FixParameter(5, fPreFit_pol2->GetParameter(2)); //pol2  coefficients, to be kept fixed from the perfit    
      fitter_eff_range->FixParameter(6, fPreFit_pol2->GetParameter(3));      
      fitter_eff_range->FixParameter(7, fPreFit_pol2->GetParameter(4)); 
      fitter_eff_range->SetParNames("Re(f0)_fd0_fixed","Im(f0)_d0_fixed","d0", "N_d", "w_c", "a", "b", "c");
      fitter_eff_range->SetLineColor(kBlue);

      std::cout << "\n---------------------------------\n Total fit pol2 fixed d0 \n--------------------------------\n" << endl;
      h_HM->Fit(fitter_eff_range, "MR", "", 0, fit_max);
      
      double chi_fitter_eff_range = fitter_eff_range->GetChisquare()/fitter_eff_range->GetNDF();
      std::cout << "Chi reduced total fit " << chi_fitter_eff_range << endl;
      std::cout<<"Re scat len after pol 2 fit"  <<LKFullCF.GetCk()->GetPotPar(0)<<endl;
      std::cout<<"Re scat len after pol 2 fit"  <<LKModel->GetPotPar(0)<<endl;
      ...
      fitter_bg->SetLineColor(kGreen);
      fitter_bg->SetLineStyle(6);
      fitter->Write("fitter");
      fitter_bg->Write("bacgkround");
      fitter_eff_range->Write("fitter_fixed_eff_range");
      ...
      h_HM->GetFunction("fitter_eff_range")->SetBit(TF1::kNotDraw);

      TCanvas *c_fitter = new TCanvas();
      h_HM->Draw("");
      fitter_bg->Draw("same");
      hint->Draw("e3 same");
      hint->SetLineColor(kGreen);
      hint->SetMarkerColorAlpha(0,0);
      hint->SetFillColorAlpha(kGreen, 0.35);
      h_HM->GetXaxis()->SetRangeUser(0, 500);
      h_HM->GetYaxis()->SetRangeUser(0.79, 1.2);
      h_HM->SetTitle("; k* (MeV/c); C(k*)");
      auto legend_fit = new TLegend(0.65, 0.65, 0.88, 0.88, NULL, "brNDC");
      legend_fit->AddEntry(h_HM, "Data", "LEP");
      legend_fit->AddEntry(hint, "Femtoscopic fit");
      legend_fit->AddEntry(fitter_bg, "Background");
      legend_fit->SetTextSize(gStyle->GetTextSize()*0.6); 
      legend_fit->SetBorderSize(0);
      legend_fit->Draw();
      gStyle->SetOptStat(0);
      c_fitter->Print(TString::Format("%s/Lednic_fit_bg_%s.pdf", path_results, pair[i]));
    ...
    if(pol3BL == true){
        TF1* fitter_pol3 = new TF1("fitter_pol3", fit_functionLedn_pol3, 0, fit_max, NumberOfFitPars);            
        fitter_pol3->SetParameter(0, scattlenRE);
        fitter_pol3->SetParameter(1, scattlenIM);            
        fitter_pol3->SetParameter(2, effrange);        
        fitter_pol3->SetParameter(3, fPreFit_pol3->GetParameter(0)); //constant of normalization           
        fitter_pol3->SetParameter(4, fPreFit_pol3->GetParameter(1)); //weight of common             
        fitter_pol3->FixParameter(5, fPreFit_pol3->GetParameter(2)); //pol3  coefficients, to be kept fixed from the perfit    
        fitter_pol3->FixParameter(6, fPreFit_pol3->GetParameter(3));
        fitter_pol3->FixParameter(7, fPreFit_pol3->GetParameter(4));      
        fitter_pol3->SetParNames("Re(f0)","Im(f0)","d0", "N_d", "w_c", "a", "c", "d");
        fitter_pol3->SetLineColor(kViolet);  

        std::cout << "\n---------------------------------\n Total fit pol3 \n--------------------------------\n" << endl;
        h_HM->Fit(fitter_pol3, "MR", "", 0, fit_max);
        ...
        TF1* fitter_bg_pol3 = new TF1("fitter_bg_pol3", PreFit_pol3, 0, 2500, 5);
        ...
        fitter_bg_pol3->FixParameter(4, fitter_pol3->GetParameter(7));
        fitter_bg_pol3->SetLineColor(fitter_pol3->GetLineColor());
        fitter_bg_pol3->SetLineStyle(6);

        fitter_pol3->Write("fitter pol3");
        fitter_bg_pol3->Write("background pol3");
    ...
    if(pol1BL == true){
      TF1* fitter_pol1 = new TF1("fitter_pol1", fit_functionLedn, 0, fit_max, NumberOfFitPars);            
      ...
      std::cout << "\n---------------------------------\n Total fit pol1 \n--------------------------------\n" << endl;
      h_HM->Fit(fitter_pol1, "MR", "", 0, fit_max);
    ...
    if(polpb == true){
      ...
      TF1* fitter_pb = new TF1("fitter_pb", fit_functionLedn, 0, fit_max, NumberOfFitPars);            
      ...
      h_HM->Fit(fitter_pb, "MR", "", 0, fit_max);
